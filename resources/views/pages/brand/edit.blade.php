...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('brand.index') }}" class="d-none d-sm-inline-block btn btn-sm btn-warning shadow-sm"><i
    ...
    <div class="row">
        <div class="col-md-12 col-xl-12">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('brand.update_brand', $brand->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" value="{{ $brand->name }}" name="name" required />
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="store_id" id="store_id" class="form-control" required>
                                        ...
                                        @foreach ($store as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $brand->store->id == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
